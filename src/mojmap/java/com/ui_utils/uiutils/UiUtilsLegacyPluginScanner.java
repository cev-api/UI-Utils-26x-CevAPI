...
public final class UiUtilsLegacyPluginScanner {
	...
	private static final List<UiUtilsPluginScanner.PluginResultRow> lastRows = new ArrayList<>();
	...
	public static void onSuggestionsPacket(ClientboundCommandSuggestionsPacket packet) {
		...
		try {
			...
			UiUtils.LOGGER.warn("Failed to parse legacy plugin scan response.", e);
		}
	...
	private static void finishAndPrint() {
		...
		lastRows.clear();
		Component line = Component.literal("[UI-Utils] Legacy Plugins (" + foundPlugins.size() + "): ")
			.withColor(0x55CCFF);
		boolean first = true;
		for (String plugin : foundPlugins) {
			...
			lastRows.add(new UiUtilsPluginScanner.PluginResultRow(
